...
export default function ResetPassword() {
  ...
  const  handleSubmit=async(e)=>{
    ...
    try{
      const { data } = await axios.put(`${URL}/login/resetpassword/${email}`, pswd)
  ...
  return (
    ...
    <Typography component={"p"}align="center" sx={{ m: 1 }} >Enter your new password</Typography>
      ...
      <TextField
                    ...
                    sx={{ m: 1,color:"white" }}
                  />
         {error.password&&(<Typography component={"p"} sx={{ fontSize: 13,color:"red" }} >{error.password}</Typography>)}
      <TextField
        type="password"
        ...
        sx={{ m: 1,color:"white"}}
                />
                  {error.password_confirm&&(<Typography component={"p"} sx={{ fontSize: 13,color:"red" }} >{error.password_confirm}</Typography>)}
